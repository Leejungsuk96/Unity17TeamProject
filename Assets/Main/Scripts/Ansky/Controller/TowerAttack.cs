...
public class TowerAttack : MonoBehaviour
{
    ...
    private void Attack()
    {
        Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, _attackData.range);
        ...
        foreach(Collider2D collider in colliders)
        {
            if (collider.CompareTag("Enemy") && CurrentBulletcount < BulletPerShot)
            {
                ...
                bullet.SetTarget(collider.transform, _stats.CurrentStats.attackSO.power);
                CurrentBulletcount++;
            }
        }
    }
